...
<body>
            ...
            <div class="shadow overflow-hidden sm:rounded-md">
              <div class="px-4 py-5 bg-white sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    <label for="nationality_id" class="block text-sm font-medium text-gray-700">Nationality ID</label>
                    <input type="number" required name="nationality_id" id="nationality_id" min="1" max="15" class="mt-1 text-gray-700 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    <label for="mission_id" class="block text-sm font-medium text-gray-700">Mission ID</label>
                    <input type="number" required name="mission_id" id="mission_id" min="1" max="10" class="mt-1 text-gray-700 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                  </div>
                </div>
  ...
  <div class="block text-center py-8">
    ...
    foreach ($pdo->query('SELECT * FROM target') as $target) {
      echo '<form id="' . $target['id'] . '" action="./actions/targetDelete.php" method="POST">';
      ...
      echo '</p>';
      echo '<button type="submit" value="' . $target['code_name'] . '" name="delete" class="mt-2 p-2 rounded-lg bg-red-600">';
      echo 'Delete entity';
      echo '</button>';
      echo '</form>';
      echo '</div>';
